        ...
        $(table.body()).on('click', 'td', function () {
            ...
            if ((settings.columns && settings.columns.indexOf(currentColumnIndex) > -1) || (!settings.columns)) {
                ...
                if (!$(cell).find('input').length && !$(cell).find('select').length && !$(cell).find('textarea').length) {
                    // Input CSS
                    var input = getInputHtml(currentColumnIndex, settings, oldValue, rowId);
                    $(cell).html(input.html);
                    if (input.focus) {
                        $('#ejbeatycelledit'+rowId).focus();
                    }
                }
            }
        });
...
function getInputHtml(currentColumnIndex, settings, oldValue, rowId) {
	            ...
	            /*var self = jQuery('.datepick').datepicker( 
                    {
					 ...
					 var fpStartTime = flatpickr(".datepick", {
						enableTime: false,
						dateFormat: "d/m/Y",
                         "locale": "it",
                         ...
                         parseDate: function (dateStr) {
                             debugger;
                             //var appo = moment("", "DD/MM/YYYY").format('YYYY-MM-DD[T]HH:mm:ss');
                            // var appo = moment.parseZone(dateStr).format('DD/MM/YYYY')
                            // return appo;
